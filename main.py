...
def calculate_joint_entropy(column1: pd.Series, column2: pd.Series) -> float:
    ...
    joint_entropy = -sum(probabilities * np.log2(probabilities))
...
def caluclateMutualInformation(column1: pd.Series, column2: pd.Series) -> float:
    """I(X;Y) = h(X) - h(X|Y) = H(X) + H(Y)  -H(X,Y)
...
def conditionalPrivacyScore(column1: pd.Series, column2: pd.Series) -> float:
    ...
    Returns:
        float: _description_
    """
    return 1 - math.pow(2, -caluclateMutualInformation(column1, column2))
...
def calculate_pointwise_mutual_information(column1: pd.Series, column2: pd.Series, value: float) -> float:
    ...
    return caluclateMutualInformation(column1, column2_binary)
...
def calculate_ITPR(X: pd.Series, Y: pd.Series) -> float:
    ...
    # ITPR(Y) over X
    max_itpr_value = 0.0
    for y_value in unique_values_Y:
...
if __name__ == "__main__":
    ...
    print("Mutual information", caluclateMutualInformation(column_X, column_Y))
